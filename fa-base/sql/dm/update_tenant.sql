...
CREATE INDEX idx_tn_tenant_name ON tn_tenant (name);
CREATE INDEX idx_tn_tenant_status ON tn_tenant (status);
...
CREATE INDEX idx_tn_tenant_user_tenant_id ON tn_tenant_user (tenant_id);
CREATE INDEX idx_tn_tenant_user_user_id ON tn_tenant_user (user_id);
CREATE INDEX idx_tn_tenant_user_status ON tn_tenant_user (status);
...
-- 部门表增加租户ID
ALTER TABLE base_department ADD tenant_id varchar(32) NULL;
COMMENT ON COLUMN base_department.tenant_id IS '租户ID';

-- 角色表增加类型和租户ID
ALTER TABLE base_rbac_role ADD type int NULL;
COMMENT ON COLUMN base_rbac_role.type IS '类型：1全局超管/2全局/3租户';

ALTER TABLE base_rbac_role ADD tenant_id varchar(32) NULL;
