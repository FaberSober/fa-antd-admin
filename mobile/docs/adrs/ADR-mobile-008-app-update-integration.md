...
| 14 | `fa-core-mobile` | App-PLUS 网络诊断日志 | 可选输出请求与响应信息，敏感值脱敏 | 执行开发 | ✅已完成 |
...
- 原生 App 默认关闭诊断日志；关于页图标连续点击 7 次可在当前运行期间统一启停 Demo 入口、HTTP/WGT 诊断日志和 Eruda Console。
- 设置 `VITE_APP_HTTP_LOG_ENABLED=true` 可在构建时默认打开 HTTP/WGT 诊断日志；关于页隐藏入口可在运行时打开。
- 同一开关下输出更新包下载、摘要校验、WGT/APK 安装阶段及原生错误码和错误信息；诊断记录同时转发到 Eruda Console，最多暂存最近 100 条。
- Eruda 只启用 Console 工具并关闭 JavaScript 执行，调试状态不持久化，应用重启后默认关闭。
