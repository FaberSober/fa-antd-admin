...
| `fa-base-mobile` 账户 | 个人资料查看 | 展示用户、账号、联系方式、状态和当前租户 | 执行开发 | 🕒待处理 |
| `fa-base-mobile` 账户 | 个人资料编辑 | 修改头像、姓名、手机号和邮箱，保存后同步 Store | 执行开发 | 👀待确认 |
| `fa-base-mobile` 账户 | 修改密码 | 校验表单并调用现有接口，成功后重新登录 | 执行开发 | 🕒待处理 |
| `fa-base-mobile` 认证 | 退出登录状态同步 | 清理 Token、用户和租户上下文，避免重复提交 | 执行开发 | 🟡进行中 |
| `fa-base-mobile` 消息 | 真实消息列表 | 接入分页、全部/未读筛选、刷新、空态和错误重试 | 执行开发 | 🕒待处理 |
| `fa-base-mobile` 消息 | 已读与未读数 | 支持单条/全部已读，同步 Header 和 Tab Bar 角标 | 执行开发 | 🕒待处理 |
