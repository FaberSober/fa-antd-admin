      ...
      ...BaseTableUtils.genUpdateColumns(sorter),
      {
        ...
        render: (text: string, record: Admin.Job) => (
          <Space>
            ...
            <ShiroPermissionContainer roleList={user.elements} permission="jobManager:btn_edit">
              <Popconfirm title={record.status === FaberBase.TrueOrFalse.FALSE ? '确定启动任务?' : '确定停止任务?'} onConfirm={() => handleJobStatus(record)} getPopupContainer={() => document.body}>
                <a>
                  {record.status === FaberBase.TrueOrFalse.FALSE ? (
                    <span>
                      <PlayCircleOutlined />
                      启动
                    </span>
