        ...
        for (d in c)
          if (c.hasOwnProperty(d))
            (f = p(b.buttonOptions, c[d])),
              (e = a.renderer.button(f.text, 0, 0, i).attr({ width: f.width, height: f.height }).css(f.style).add()),
              (e.handler = f.onclick),
    ...
    getBox: function (a) {
      ...
      l(a || this.options.data, function (a) {
        for (var i = a.path, h = i.length, j = !1, g = Number.MIN_VALUE, m = Number.MAX_VALUE, k = Number.MIN_VALUE, l = Number.MAX_VALUE; h--; )
          typeof i[h] === 'number' &&
            !isNaN(i[h]) &&
            (j ? ((g = Math.max(g, i[h])), (m = Math.min(m, i[h]))) : ((k = Math.max(k, i[h])), (l = Math.min(l, i[h]))), (j = !j));
        a._maxX = g;
        a._minX = m;
        a._maxY = k;
        a._minY = l;
