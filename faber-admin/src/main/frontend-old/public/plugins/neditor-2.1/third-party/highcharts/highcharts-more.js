  ...
  h.waterfall = x(h.column, {
    ...
    processData: function (a) {
      ...
      for (j = 0; j < g; j++)
        (k = b[j]),
          (d = c && c[j] ? c[j] : {}),
          k === 'sum' || d.isSum ? (b[j] = i) : k === 'intermediateSum' || d.isIntermediateSum ? ((b[j] = e), (e = f)) : ((i += k), (e += k)),
          (h = Math.min(i, h)),
          (l = Math.max(i, l));
    ...
    if (b.isRadial) {
      ...
      for (h = b.length; h--; )
        (e = b[h]),
          (a = e.barX + g),
          (e.shapeType = 'path'),
          (e.shapeArgs = { d: f.symbols.arc(d[0], d[1], c - e.plotY, null, { start: a, end: a + e.pointWidth, innerR: c - r(e.yBottom, c) }) }),
          this.toXY(e);
    }
  });
