...
class Outline extends BaseTreeApi<Article.Outline, number> {
	allBookIdTree = (bookId: number): Promise<Fa.Response<Fa.TreeNode[]>> => this.get(`${bookId}/allTree`);

	findDetail = (id: number): Promise<Fa.Response<Article.OutlineDetailVo>> => this.get(`findDetail/${id}`);
}
