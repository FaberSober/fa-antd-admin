            ...
            case 'A':
              for (; !b.isCommandOrEnd(); ) {
                ...
                o =
                  (o == j ? -1 : 1) *
                  Math.sqrt(
                    (Math.pow(h, 2) * Math.pow(l, 2) - Math.pow(h, 2) * Math.pow(n.y, 2) - Math.pow(l, 2) * Math.pow(n.x, 2)) /
    ...
    a.Element.radialGradient = function (c) {
      ...
      this.getGradient = function (a, b) {
        ...
        this.attribute('fx').hasValue() &&
          (g = this.gradientUnits == 'objectBoundingBox' ? c.x() + c.width() * this.attribute('fx').numValue() : this.attribute('fx').Length.toPixels('x'));
        this.attribute('fy').hasValue() &&
          (j = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('fy').numValue() : this.attribute('fy').Length.toPixels('y'));
