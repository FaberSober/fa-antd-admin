...
export default function FormItemDecoHrProperty() {
  return (
    <div>
      <Form.Item name="deco_hrColor" label="分割线颜色" getValueFromEvent={(color) => color.toHexString()}>
        <ColorPicker showText format="hex" />
      </Form.Item>

      <Form.Item name="deco_hrThickness" label="分割线粗细">
        <InputNumber min={1} max={10} placeholder="1" addonAfter="px" style={{ width: '100%' }} />
      </Form.Item>

      <Form.Item name="deco_hrStyle" label="分割线样式">
        <Select
          placeholder="选择分割线样式"
          options={[
      ...
      <Form.Item name="deco_hrMarginTop" label="上边距">
        <InputNumber min={0} max={100} placeholder="16" addonAfter="px" style={{ width: '100%' }} />
      </Form.Item>

      <Form.Item name="deco_hrMarginBottom" label="下边距">
        <InputNumber min={0} max={100} placeholder="16" addonAfter="px" style={{ width: '100%' }} />
      </Form.Item>
    </div>
  );
