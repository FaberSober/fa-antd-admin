...
(function () {
  ...
  Parser = (function () {
    ...
    function Parser() {
      this.commonWhiteList =
        "kbd|b|i|strong|em|sup|sub|br|code|del|a|hr|small|span";
    ...
    Parser.prototype.parseInline = function (
      text,
      whiteList,
      clearHolders,
      enableAutoLink
      ...
      text = text.replace(
        ...
        (function (_this) {
          return function () {
            ...
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            if (
              ("|" + _this.commonWhiteList + "|" + whiteList + "|").indexOf(
      ...
      text = text.replace(
        ...
        (function (_this) {
          return function () {
            ...
            escaped = _this.escapeBracket(matches[1]);
            result =
              _this.definitions[matches[2]] != null
                ? '<img src="' +
                  _this.definitions[matches[2]] +
                  '" alt="' +
                  escaped +
                  '" title="' +
                  escaped +
      ...
      text = text.replace(
        ...
        (function (_this) {
          return function () {
            ...
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return (
              "<strong>" + _this.parseInlineCallback(matches[2]) + "</strong>"
            );
          };
        })(this)
      );
    ...
    Parser.prototype.parseBlock = function (text, lines) {
      ...
      for (key = l = 0, len1 = lines.length; l < len1; key = ++l) {
        ...
        if (!!(matches = line.match(/^(\s*)(~|`){3,}([^`~]*)$/i))) {
          if (this.isBlock("code")) {
            ...
            if (this.isBlock("list")) {
              space = block[3];
              isAfterList =
                (space > 0 && matches[1].length >= space) ||
        ...
        if (
          ...
          !!(matches = line.match(new RegExp("</(" + special + ")>\\s*$", "i")))
        ) {
          tag = matches[1].toLowerCase();
      ...
      while (blocks[key] != null) {
        ...
        if ("normal" === type) {
          ...
          if (
            ...
            if (
              prevBlock[0] === nextBlock[0] &&
              types.indexOf(prevBlock[0]) >= 0
            ) {
              blocks[key - 1] = [
                prevBlock[0],
                prevBlock[1],
    ...
    Parser.prototype.parseCode = function (lines, parts) {
      ...
      if (str.match(/^\s*$/)) {
        ...
        return (
          '<pre data-start="1" class="line-numbers"><code' +
          (!!lang ? ' class="' + lang + '"' : "") +
          (!!rel ? ' rel="' + rel + '"' : "") +
          ">" +
          htmlspecialchars(str) +
          "</code></pre>"
        ); // class="line-numbers" 代码高亮 @Robbie
      }
    };
    ...
    Parser.prototype.parseList = function (lines) {
      ...
      for (m = 0, len2 = rows.length; m < len2; m++) {
        ...
        if (row instanceof Array) {
          ...
          if (space !== minSpace) {
            leftLines.push(
              line.replace(new RegExp("^\\s{" + secondMinSpace + "}"), "")
            );
          } else {
          ...
          leftLines.push(
            row.replace(new RegExp("^\\s{" + secondMinSpace + "}"), "")
          );
        }
      }
      if (leftLines.length > 0) {
      ...
      for (key = j = 0, len = lines.length; j < len; key = ++j) {
        ...
        for (l = 0, len1 = rows.length; l < len1; l++) {
          ...
          if (row.length > 0) {
            last += 1;
            columns[last] = [
              columns[last] != null ? columns[last][0] + 1 : 1,
              row,
            ];
    ...
    Parser.prototype.parseHtml = function (lines, type) {
      lines = lines.map(
        (function (_this) {
          return function (line) {
            return _this.parseInline(
              line,
              _this.specialWhiteList[type] != null
                ? _this.specialWhiteList[type]
                : ""
            );
          };
        })(this)
    ...
    Parser.prototype.cleanUrl = function (url) {
      ...
      if (
        !!(matches = url.match(
          /^\s*((http|https|ftp|mailto):[x80-xff_a-z0-9-\.\/%#@\?\+=~\|\,&\(\)]+)/i
        ))
      ) {
        matches[1];
      }
      if (
        !!(matches = url.match(/^\s*([x80-xff_a-z0-9-\.\/%#@\?\+=~\|\,&]+)/i))
      ) {
        return matches[1];
      } else {
        return "#";
