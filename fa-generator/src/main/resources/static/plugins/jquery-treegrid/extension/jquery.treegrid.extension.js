    ...
    target.getRootNodes = function (data) {
      ...
      $.each(data, function (index, item) {
        ...
        if (
          !item[options.parentCode] ||
          (_root
            ? item[options.parentCode] == options.rootCodeValue
            : _defaultRootFlag)
        ) {
          result.push(item);
        }
        ...
        success: function (data, textStatus, jqXHR) {
          ...
          target.treegrid({
            treeColumn: options.expandColumn
              ? options.expandColumn
              : hasSelectItem
              ? 1
              : 0, //如果有radio或checkbox默认第二列层级显示，当前是在用户未设置的提前下
          ...
          target
            ...
            .click(function () {
              ...
              if (options.clickRow) options.clickRow();
            });
        },
  ...
  $.fn.bootstrapTreeTable.methods = {
    ...
    getSelections: function (target, data) {
      ...
      var _ipt = target
        .find("tbody")
        .find("tr")
        .find("input[name='select_item']:checked");
      var chk_value = [];
      // 如果是radio
      if (_ipt.attr("type") == "radio") {
