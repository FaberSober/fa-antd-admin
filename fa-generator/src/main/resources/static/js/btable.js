...
layui.define(["element", "common", "paging", "form"], function (exports) {
  ...
  BTable.prototype.render = function () {
    ...
    for (var i = 0; i < columns.length; i++) {
      if (columns[i].sortable) {
        th +=
          ...
          '">' +
          columns[i].fieldName +
          '<div class="btable-order"><div class="up" title="升序"><i class="fa fa-sort-asc" aria-hidden="true"></i></div ><div class="down" title="倒序"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div></th>';
      } else {
    ...
    if (_config.checkbox && !_config.singleSelect) {
      th =
        '<th style="width:28px;"><input type="checkbox" lay-filter="allselector" lay-skin="primary" /></th><th style="width:28px;">序号</th>' +
        th;
    } else if (_config.checkbox) {
      th =
        '<th style="width:28px;">#</th><th style="width:28px;">序号</th>' + th;
    } else {
      th = '<th style="width:28px;">序号</th>' + th;
    }
    var tpl = '<div class="btable">';
    if (
      _config.skin !== undefined &&
      (_config.skin === "line" ||
        _config.skin === "row" ||
        _config.skin === "nob") &&
      _config.even
      ...
      tpl +=
        '<table class="layui-table layui-form" lay-even lay-skin="' +
        _config.skin +
        '">';
    } else if (
      _config.skin !== undefined &&
          ...
          $(_config.elem)
            ...
            .each(function () {
              ...
              $that
                .children("td")
                .eq(0)
                .children("input[type=checkbox]")[0].checked = elem.checked;
              form.render("checkbox");
            });
        });
        if (_config.checkbox) {
                ...
                if (_config.singleSelect) {
                  $that.siblings().each(function () {
                    $(this)
                      .children("td")
                      .eq(0)
                      .children("input[type=checkbox]")[0].checked = false;
                  });
                  $that
                    .children("td")
                    .eq(0)
                    .children("input[type=checkbox]")[0].checked = true;
                } else {
                  //获取当前的状态
                  var currState = $that
                    .children("td")
                    .eq(0)
                    .children("input[type=checkbox]")[0].checked;
                  $that
                    .children("td")
                    .eq(0)
                    .children("input[type=checkbox]")[0].checked = !currState;
                  ...
                  $that
                    ...
                    .each(function () {
                      var $that = $(this);
                      if (
                        $that.children("td:first-child").children("input")[0]
                          .checked
                      ) {
                        cbxCount++;
                      }
  ...
  BTable.prototype.getSelected = function (callback) {
    var that = this;
    var _config = that.config;
    if (!_config.singleSelect) return callback({});
  ...
  function getTpl(options) {
    ...
    if (options.checkbox) {
      tds =
        '<td><input type="checkbox" data-item="id" data-id="{{ item.' +
        options.field +
        ' }}" lay-skin="primary" /></td><td>{{ (index+1) }}</td>' +
        tds;
    } else {
      tds =
        '<td style="display:none;"><input type="hidden" data-item="id" value="{{ item.' +
        options.field +
        ' }}" data-id="{{ item.' +
        options.field +
        ' }}" name="id" /></td><td>{{ (index+1) }}</td>' +
        tds;
    }
    tpl += "<tr>" + tds + "</tr>";
    tpl += "{{# }); }}";
    tpl += "{{# }else{ }}";
    var colLength =
      options.checkbox && !options.singleSelect
        ? columns.length + 1
        : columns.length;
