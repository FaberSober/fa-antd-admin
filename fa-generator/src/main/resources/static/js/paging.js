...
layui.define(["layer", "laypage", "laytpl"], function (exports) {
  ...
  Paging.prototype.init = function (options) {
    ...
    if (
      _config.type.toUpperCase() !== "GET" &&
      _config.type.toUpperCase() !== "POST"
    ) {
  ...
  Paging.prototype.get = function (options) {
    ...
    $.ajax({
      ...
      success: function (result, status, xhr) {
        if (loadIndex !== undefined) layer.close(loadIndex); //关闭等待层
        if (result.rel) {
          //获取模板
          var tpl =
            _config.tempType === 0
              ? $(_config.tempElem).html()
              : _config.tempElem;
          //渲染数据
          laytpl(tpl).render(result, function (html) {
            if (_config.renderBefore) {
              _config.renderBefore(
                html,
                function (formatHtml) {
